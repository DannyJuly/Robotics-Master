...
class MoveArm(object):
	...
	########INSERT YOUR RRT MOTION PLANNING HERE##########
        ...
        # for i in range(self.num_joints):
        #     if self.q_current[i] < -numpy.pi:
        #         self.q_current[i]+=2*numpy.pi
        #     if self.q_current[i] > numpy.pi:
        #         self.q_current[i]-=2*numpy.pi
        ...
        while time.time() < t_end or self.q_current != q_goal:
            if (self.num_joints==7):
                ...
                q_random = [numpy.random.uniform(-2,2)*numpy.pi for i in range(self.num_joints)]
            ...
            if self.collision_free(nearest_node,next_node) == True:
                ...
                #print(q_new.q)
                print(k)
                node_list.append(q_new)#({'position':next_node,'parent':min_index})
